...
TEST_F(ScalarEvolutionsTest,
  ...
  runWithSE(*M, "foo", [](Function &F, LoopInfo &LI, ScalarEvolution &SE) {
    ...
    EXPECT_EQ(SE.getExistingSCEV(Range), RangeScev);
    EXPECT_EQ(SE.getExistingSCEV(Multiple), MultipleScev);
    EXPECT_EQ(SE.getExistingSCEV(AtScope), AtScopeScev);
...
TEST_F(ScalarEvolutionsTest, ForgetValueWithUncachedIntermediateValues) {
  LLVMContext C;
  SMDiagnostic Err;
  std::unique_ptr<Module> M =
      parseAssemblyString("define i32 @foo(i32 %x) { "
                          "  %add1 = add i32 %x, 1 "
                          "  %add2 = add i32 %add1, 1 "
                          "  %add3 = add i32 %add2, 1 "
                          "  ret i32 %add3 "
