...
const char *
RemniwRISCVTargetLowering::getTargetNodeName(unsigned Opcode) const {
  switch (Opcode) {
  case RemniwRISCVISD::CALL:
    return "RemniwRISCVISD::CALL";
  case RemniwRISCVISD::RET_FLAG:
    return "RemniwRISCVISD::RET_FLAG";
  default:
    return nullptr;
  }
}
...
SDValue
RemniwRISCVTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  ArgCCInfo.AnalyzeCallOperands(Outs, CC_RemniwRISCV);
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    if (VA.isRegLoc()) {
      ...
      if (StackPtr.getNode() == nullptr)
        StackPtr = DAG.getCopyFromReg(Chain, DL, RemniwRISCV::X2,
  ...
  Chain = DAG.getNode(RemniwRISCVISD::CALL, DL, NodeTys, Ops);
  InFlag = Chain.getValue(1);
  ...
  RetCCInfo.AnalyzeCallResult(Ins, RetCC_RemniwRISCV);
...
SDValue RemniwRISCVTargetLowering::LowerFormalArguments(
...
SDValue RemniwRISCVTargetLowering::LowerReturn(
    SDValue Chain, CallingConv::ID CallConv, bool IsVarArg,
  ...
  CCState CCInfo(CallConv, IsVarArg, DAG.getMachineFunction(), RVLocs,
                 *DAG.getContext());
  CCInfo.AnalyzeReturn(Outs, RetCC_RemniwRISCV);
  ...
  return DAG.getNode(RemniwRISCVISD::RET_FLAG, DL, MVT::Other, RetOps);
}
